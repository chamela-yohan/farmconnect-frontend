...
import { useConversationByBooking } from "@/lib/api/chat";
import { BookingStatus } from "@/types/booking";
import { Loader2, Calendar, User, CheckCircle2, XCircle, Clock, AlertTriangle, MessageSquare } from "lucide-react";
import { OrderActionModal } from "@/components/orders/OrderActionModal";
import { ChatDrawer } from "@/components/chat/ChatDrawer";
import { toast } from "sonner";

// Extracted sub-component to isolate chat state and hooks per booking
function BookingCard({ booking }: { booking: any }) {
  ...
  // Fetches the conversation ID specifically for this booking
  const { data: conversationId } = useConversationByBooking(booking.id);

  const formatDate = (dateString: string) =>
    new Intl.DateTimeFormat("en-LK", { month: "short", day: "numeric", year: "numeric" }).format(new Date(dateString));

  const handleActionClick = (type: "ACCEPT" | "REJECT") => {
    setActionType(type);
    setModalOpen(true);
  };
  ...
  return (
    <>
      <div className="bg-card border border-border rounded-xl overflow-hidden hover:shadow-md transition-shadow">
        ...
        {/* Actions */}
        <div className="px-5 py-3 bg-muted/20 border-t border-border flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          <div className="flex gap-2">
            {booking.status === "PENDING" && (
              <>
                <button onClick={() => handleActionClick("REJECT")} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 transition-colors flex items-center gap-2">
                  <XCircle className="w-4 h-4" /> Reject
                </button>
                <button onClick={() => handleActionClick("ACCEPT")} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2">
          ...
          <button
            onClick={() => {
              if (conversationId) {
                ...
                toast.info("Chat will be available once the booking is accepted.");
              }
            }}
          ...
          >
            ...
            {conversationId ? "Contact Buyer" : "Chat Unavailable"}
          </button>
        </div>
      </div>
      ...
      {modalOpen && (
        <OrderActionModal
          ...
          orderNumber={booking.productTitle}
          isPending={updateMutation.isPending}
        />
      )}

      {/* Chat Drawer scoped to this specific booking */}
      <ChatDrawer
        isOpen={isChatOpen}
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {/* Map over bookings and render the isolated BookingCard */}
