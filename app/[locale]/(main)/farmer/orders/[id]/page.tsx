...
import { Loader2, Package, MapPin, Store, MessageSquare, Download, ChevronLeft, User } from "lucide-react";
...
export default function FarmerOrderDetailsPage() {
  ...
  if (isError || !order) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <Link href={`/${locale}/farmer/orders`} className="text-primary hover:underline mt-4 inline-flex items-center justify-center gap-2">
          <ChevronLeft className="w-4 h-4" /> Back to Received Orders
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-6">
        <Link href={`/${locale}/farmer/orders`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
          <ChevronLeft className="w-4 h-4" /> Back to Received Orders
        </Link>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="bg-muted/50 px-6 py-4 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Received on {formatDate(order.createdAt)}</p>
          </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-8">
            <div>
              <h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                <Package className="w-5 h-5" /> Requested Items
              </h2>
              ...
              <div className="p-4 bg-muted/30 rounded-lg border border-border space-y-3">
                ...
                {order.buyerNotes && (
                  <div className="pt-3 border-t border-border bg-blue-50/50 p-3 rounded-md -mx-3 -mb-3 px-4 pb-4">
                    ...
                    <p className="text-sm text-blue-700 italic mt-1">"{order.buyerNotes}"</p>
                  </div>
                )}
                
                {order.farmerNotes && (
                  <div className="pt-3 border-t border-border">
                    <p className="text-sm font-medium text-foreground">Your Notes to Buyer:</p>
          ...
          <div className="space-y-6">
            <div className="p-5 bg-card border border-border rounded-xl">
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Buyer Details</h3>
