...
import { Loader2, Package, MapPin, Store, CheckCircle2, XCircle, Truck } from "lucide-react";
...
export default function FarmerOrdersPage() {
  ...
  const tabs = ["ALL", "PENDING", "ACCEPTED", "PREPARING", "OUT_FOR_DELIVERY", "DELIVERED", "REJECTED"];

  const handleActionClick = (orderId: string, orderNumber: string, type: "ACCEPT" | "REJECT") => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Manage incoming requests and update order statuses.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-destructive">Failed to load orders.</div>
                ...
                <div className="bg-muted/50 px-5 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground">
                      From: {order.buyerName} {order.buyerMobile && `(${order.buyerMobile})`}
                    </span>
                  </div>
                ...
                <div className="p-5">
                    ...
                    {order.items.map((item) => (
                      <div key={item.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate text-foreground">{item.productTitle}</p>
                          <p className="text-xs text-muted-foreground">
                            Requested: {item.requestedQty} {item.attributesSnapshot?.unit || 'units'}
                            {item.requestedQty !== item.approvedQty && ` | Approved: ${item.approvedQty}`}
                          </p>
                        </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground pt-3 border-t border-border mb-4">
                    {order.deliveryMethod === 'DELIVERY' ? (
                      <><MapPin className="w-3 h-3" /> Deliver to: {order.deliveryAddress}</>
                    ) : (
                      <><Store className="w-3 h-3" /> Buyer will pickup from farm</>
                    )}
                    {order.buyerNotes && <span className="ml-2 italic">| Note: "{order.buyerNotes}"</span>}
                  </div>
                  ...
                  {actions.length > 0 && (
                    <div className="flex gap-2 pt-2">
                      {actions.includes("ACCEPT") && (
                        <button
                          onClick={() => handleActionClick(order.id, order.orderNumber, "ACCEPT")}
                      ...
                      {actions.includes("REJECT") && (
                        <button
                          onClick={() => handleActionClick(order.id, order.orderNumber, "REJECT")}
                        ...
                        >
                          <XCircle className="w-4 h-4" /> {order.status === 'PENDING' ? 'Reject' : 'Cancel'}
                        </button>
                      )}
                      {actions.includes("PREPARING") && (
                        <button
                          onClick={() => updateMutation.mutateAsync({ orderId: order.id, request: { newStatus: "PREPARING" } })}
                      ...
                      {actions.includes("OUT_FOR_DELIVERY") && (
                        <button
                          onClick={() => updateMutation.mutateAsync({ orderId: order.id, request: { newStatus: "OUT_FOR_DELIVERY" } })}
                      ...
                      {actions.includes("DELIVERED") && (
                        <button
                          onClick={() => updateMutation.mutateAsync({ orderId: order.id, request: { newStatus: "DELIVERED" } })}
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-muted-foreground mt-2">You have no {activeStatus === 'ALL' ? '' : activeStatus.toLowerCase()} orders at the moment.</p>
        </div>
      )}
