...
// Extracted into a sub-component so each booking manages its own chat state and hook
function BookingCard({ booking }: { booking: any }) {
  ...
  const getStatusBadge = (status: BookingStatus) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${current.color}`}
      >
        {current.icon}
        {current.label}
      </span>
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">
          Track your equipment rentals and service bookings.
        </p>
      </div>
